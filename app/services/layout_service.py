...
class MockLayoutModel:
    ...
    def detect(self, image):
        ...
        # 모든 이미지에 최소 1개 요소는 생성하도록 보장
        # 첫 번째 요소: 제목 - 항상 생성
        if height >= 20 and width >= 20:
            logger.info(f"Creating title element for {width}x{height} image")
            mock_elements.append(
                MockLayoutElement(10, 10, min(width-20, max(width//2, 100)), min(height-20, max(height//4, 40)), "title", 0.95)
            )
        else:
            logger.warning(f"Image too small for title element: {width}x{height}")
        
        # 두 번째 요소: 텍스트 - 중간 크기 이미지부터
        if height > 100 and width > 100:
            y_start = max(60, height//4)
            mock_elements.append(
                MockLayoutElement(10, y_start, width-10, min(height-10, y_start + height//3), "text", 0.85)
            )
        
        # 세 번째 요소: 텍스트 - 큰 이미지에만
        if height > 200 and width > 200:
            y_start = max(height//2, 120)
            mock_elements.append(
                MockLayoutElement(10, y_start, width-10, height-10, "text", 0.80)
            )
        
        # 안전장치: 요소가 없으면 강제로 1개 생성
        if not mock_elements and height > 0 and width > 0:
            logger.warning("No elements generated by normal rules, creating fallback element")
            mock_elements.append(
                MockLayoutElement(5, 5, min(width-10, 50), min(height-10, 30), "text", 0.8)
            )
...
class LayoutService:
    def __init__(self, model_name: str, confidence_threshold: float = 0.1):  # 매우 낮은 임계값으로 설정
    ...
    def load_model(self):
        # 디버깅을 위해 강제로 Mock 모델 사용
        logger.info("DEBUG: Forcing mock model for testing")
        ...
        try:
            ...
            try:
                self.model = lp.Detectron2LayoutModel(
                    ...
                    model_path='/Users/soonhobahng/.layoutparser/models/PubLayNet_mask_rcnn_X_101_32x8d_FPN_3x.pth',
            ...
            # 4. 최후 수단: Mock 모델 (강제로 사용하도록 임시 설정)
    ...
    def detect(self, image: np.ndarray, custom_threshold: float = None) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Layout detect called - model type: {type(self.model).__name__}")
        ...
        try:
            ...
            elements = []
            logger.info(f"Processing {len(layout_result)} layout elements")
            
            for idx, element in enumerate(layout_result):
                try:
                    x1, y1, x2, y2 = element.coordinates
                    score = element.score if hasattr(element, 'score') else threshold
                    element_type = element.type
                    
                    logger.info(f"Element {idx}: type={element_type}, bbox=({x1},{y1},{x2},{y2}), score={score}")
                    ...
                    logger.error(f"Element type: {type(element)}, Element: {element}")
                    continue
